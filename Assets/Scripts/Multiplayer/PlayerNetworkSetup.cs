...
public class PlayerNetworkSetup : MonoBehaviour
{
    ...
    void Start()
    {
        if(NetView.IsMine)
        {
            ...
            if (Network.LocalPlayer.CustomProperties.TryGetValue(MultiplayerVRConstants.AVATAR_SELECTION_NUMBER, out avatarSelectionNumber))
            {
                Debug.Log("Avatar selection number:" + (int)avatarSelectionNumber);
                NetView.RPC("InitializeSelectedAvatarModel", RpcTarget.AllBuffered, (int)avatarSelectionNumber);
            }
        ...
        {
            PlayerName_Text.text = Owner.NickName;
        }
    }
